...
const Sidebar = (props) => {
  ...
  return (
    <div
      className={`h-screen w-full max-w-[16rem] p-2 shadow-xl shadow-blue-gray-900/5 
    ...
    >
      ...
      <nav className="flex flex-col gap-1 text-base font-normal mt-4 h-body-sidebar">
        {items?.map((item, index) => {
          if (item.type === 'item') return <NavItem key={index} item={item} />;
          else return <NavGroup key={index} item={item} value={index + 1} open={open} handleOpen={setOpen} />;
        })}
      </nav>
