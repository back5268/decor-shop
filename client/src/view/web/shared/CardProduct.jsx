  ...
  return (
    <>
      ...
      <div className="mt-2 px-4 flex flex-col gap-1 font-medium">
        ...
        <div className="mt-1 flex gap-3 text-lg items-center">
          <span className="text-red-600 font-normal">{formatNumber(item.price - item.sale || 0)} đ</span>
          {item.sale && (
            <>
              <span className="text-gray-500 font-normal text-sm line-through">{formatNumber(item.price)} đ</span>
              <span className="py-1 px-2 text-xs text-red-500 rounded-lg   bg-red-100">-{Math.round((item.sale / item.price) * 100)}%</span>
            </>
          )}
        </div>
        <div className="flex items-center gap-3">
          <span className='text-red-500'>{item?.vote}</span> <Ratez value={item?.vote} />
          <span className="text-sm font-normal">Đã bán {formatNumber(item.saleNumber) || 0}</span>
        </div>
      </div>
    </>
  );
